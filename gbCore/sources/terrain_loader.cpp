...
namespace gb
{
    ...
    std::tuple<glm::ivec2, std::vector<f32>> terrain_loader::get_heights_data(const std::string& filename)
    {
        ...
        for(ui32 i = 0; i < size.x; ++i)
        {
            ...
            {
                heights[index++] = static_cast<f32>(data[(i + j * size.x) * 4 + 1]) / 255.f * terrain_container::k_raise - terrain_container::k_deep;
            }
        }
    ...
    std::string terrain_loader::get_uncompressed_vertices_mmap_filename(const std::string &filename)
    {
    ...
    std::string terrain_loader::get_compressed_vertices_mmap_filename(const std::string& filename)
    {
        std::ostringstream stringstream;
    ...
    std::string terrain_loader::get_faces_mmap_filename(const std::string& filename)
    {
        std::ostringstream stringstream;
    ...
    std::string terrain_loader::get_vbos_mmap_filename(const std::string &filename)
    {
        std::ostringstream stringstream;
    ...
    std::string terrain_loader::get_ibos_mmap_filename(const std::string &filename)
    {
        std::ostringstream stringstream;
    ...
    std::string terrain_loader::get_splatting_mask_textures_mmap_filename(const std::string &filename)
    {
        std::ostringstream stringstream;
    ...
    std::string terrain_loader::get_splatting_normal_textures_mmap_filename(const std::string& filename)
    {
        std::ostringstream stringstream;
    ...
    std::string terrain_loader::get_splatting_displace_textures_mmap_filename(const std::string& filename)
    {
        std::ostringstream stringstream;
    ...
    std::string terrain_loader::get_splatting_diffuse_textures_mmap_filename(const std::string& filename)
    {
        std::ostringstream stringstream;
    ...
    std::string terrain_loader::get_tangent_space_mmap_filename(const std::string& filename)
    {
        std::ostringstream stringstream;
    ...
    std::string terrain_loader::get_attaches_to_vbo_mmap_filename(const std::string &filename)
    {
        std::ostringstream stringstream;
    ...
    bool terrain_loader::is_compressed_vertices_mmap_exist(const std::string& filename)
    {
        std::ifstream stream(terrain_loader::get_compressed_vertices_mmap_filename(filename));
        bool exist = stream.good();
        stream.close();
        return exist;
    }
    
    bool terrain_loader::is_faces_mmap_exist(const std::string& filename)
    {
        std::ifstream stream(terrain_loader::get_faces_mmap_filename(filename));
        bool exist = stream.good();
        stream.close();
        return exist;
    }
    
    bool terrain_loader::is_vbos_mmap_exist(const std::string& filename)
    {
        std::ifstream stream(terrain_loader::get_vbos_mmap_filename(filename));
        bool exist = stream.good();
        stream.close();
        return exist;
    }
    
    bool terrain_loader::is_ibos_mmap_exist(const std::string& filename)
    {
        std::ifstream stream(terrain_loader::get_ibos_mmap_filename(filename));
        bool exist = stream.good();
        stream.close();
        return exist;
    }
    
    bool terrain_loader::is_splatting_mask_textures_mmap_exist(const std::string &filename)
    {
        std::ifstream stream(terrain_loader::get_splatting_mask_textures_mmap_filename(filename));
        bool exist = stream.good();
        stream.close();
        return exist;
    }
    
    bool terrain_loader::is_splatting_normal_textures_mmap_exist(const std::string& filename)
    {
        std::ifstream stream(terrain_loader::get_splatting_normal_textures_mmap_filename(filename));
        bool exist = stream.good();
        stream.close();
        return exist;
    }
    
    bool terrain_loader::is_splatting_displace_textures_mmap_exist(const std::string& filename)
    {
        std::ifstream stream(terrain_loader::get_splatting_displace_textures_mmap_filename(filename));
        bool exist = stream.good();
        stream.close();
        return exist;
    }
    
    bool terrain_loader::is_splatting_diffuse_textures_mmap_exist(const std::string& filename)
    {
        std::ifstream stream(terrain_loader::get_splatting_diffuse_textures_mmap_filename(filename));
        bool exist = stream.good();
        stream.close();
        return exist;
    }
    
    bool terrain_loader::is_tangent_space_mmap_exist(const std::string &filename)
    {
        std::ifstream stream(terrain_loader::get_tangent_space_mmap_filename(filename));
        bool exist = stream.good();
        stream.close();
        return exist;
    }
