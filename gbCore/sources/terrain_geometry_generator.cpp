//
//  terrain_geometry_generator.cpp
//  ie3D-Core
//
...
namespace gb
{
                                                ...
                                                const glm::ivec2& size, const std::vector<f32>& heights)
    {
        if(!terrain_loader::is_uncompressed_vertices_mmap_exist(filename) ||
           ...
           !terrain_loader::is_faces_mmap_exist(filename))
        {
        ...
        {
            if(!terrain_loader::is_vbos_mmap_exist(filename))
            {
                terrain_geometry_generator::create_vbos_data(container, filename);
            }
                                                            ...
                                                            const std::string& filename)
    {
        ...
        terrain_container::face *faces = container->get_faces();
        ...
        { // writing compressed vertices metadata
            std::ofstream stream;
            stream.open(terrain_loader::get_compressed_vertices_mmap_filename(filename), std::ios::binary | std::ios::out | std::ios::trunc);
            if(!stream.is_open())
            {
                assert(false);
            }
            ...
            for(ui32 i = 0; i < size.x * size.y; ++i)
            {
                stream.write((char*)&compressed_vertices[i], sizeof(terrain_container::compressed_vertex));
            }
            stream.close();
        }
        ...
        { // writing uncompressed vertices metadata
            std::ofstream stream;
            stream.open(terrain_loader::get_uncompressed_vertices_mmap_filename(filename), std::ios::binary | std::ios::out | std::ios::trunc);
            if(!stream.is_open())
            {
                assert(false);
            }
            ...
            for(ui32 i = 0; i < size.x * size.y; ++i)
            {
                stream.write((char*)&uncompressed_vertices[i], sizeof(terrain_container::uncompressed_vertex));
            }
            stream.close();
        }
        ...
        { // writing faces metadata
            std::ofstream stream;
            stream.open(terrain_loader::get_faces_mmap_filename(filename), std::ios::binary | std::ios::out | std::ios::trunc);
            if(!stream.is_open())
            {
                assert(false);
            }
            ...
            {
                stream.write((char*)&faces[i], sizeof(terrain_container::face));
            }
            stream.close();
        }
    }
    
    void terrain_geometry_generator::create_vbos_data(const std::shared_ptr<terrain_container>& container, const std::string& filename)
    {
        std::ofstream stream;
        stream.open(terrain_loader::get_vbos_mmap_filename(filename), std::ios::binary | std::ios::out | std::ios::trunc);
        if(!stream.is_open())
        {
            assert(false);
        }
    ...
    void terrain_geometry_generator::create_ibos_data(const std::shared_ptr<terrain_container>& container, const std::string& filename)
    {
        std::ofstream stream;
        stream.open(terrain_loader::get_ibos_mmap_filename(filename), std::ios::binary | std::ios::out | std::ios::trunc);
        if(!stream.is_open())
        {
            assert(false);
        }
        ...
        for(ui32 i = 0; i < container->get_chunks_num().x; ++i)
        {
            for(ui32 j = 0; j < container->get_chunks_num().y; ++j)
            {
                for(ui32 k = 0; k < terrain_container::e_heigtmap_chunk_lod_max; ++k)
                {
                    glm::ivec2 current_chunk_size = glm::ivec2(container->get_chunk_lod_size(static_cast<terrain_container::e_heigtmap_chunk_lod>(k)).x % 2 == 0 ?
                                                               container->get_chunk_lod_size(static_cast<terrain_container::e_heigtmap_chunk_lod>(k)).x :
                                                               container->get_chunk_lod_size(static_cast<terrain_container::e_heigtmap_chunk_lod>(k)).x - 1,
                                                               
                                                               container->get_chunk_lod_size(static_cast<terrain_container::e_heigtmap_chunk_lod>(k)).y % 2 == 0 ?
                                                               container->get_chunk_lod_size(static_cast<terrain_container::e_heigtmap_chunk_lod>(k)).y :
                                                               container->get_chunk_lod_size(static_cast<terrain_container::e_heigtmap_chunk_lod>(k)).y - 1);
                    ...
                    current_chunk_lod_start_index.x += k != terrain_container::e_heigtmap_chunk_lod_01 ? 1 : 0;
                    current_chunk_lod_start_index.y += k != terrain_container::e_heigtmap_chunk_lod_01 ? 1 : 0;
                    
                    std::vector<ui16> addition_indices;
                    if(k != terrain_container::e_heigtmap_chunk_lod_01)
                    {
                    ...
                    current_chunk_size.x -= k != terrain_container::e_heigtmap_chunk_lod_01 ? 2 : 0;
                    current_chunk_size.y -= k != terrain_container::e_heigtmap_chunk_lod_01 ? 2 : 0;
                    
                    ui32 indices_count = current_chunk_size.x * current_chunk_size.y * 6 + static_cast<ui32>(addition_indices.size());
                    
                    current_chunk_size.x += k != terrain_container::e_heigtmap_chunk_lod_01 ? 1 : 0;
                    current_chunk_size.y += k != terrain_container::e_heigtmap_chunk_lod_01 ? 1 : 0;
    ...
    void terrain_geometry_generator::generate_tangent_space(const std::shared_ptr<terrain_container>& container, const std::string& filename)
    {
        ...
        {
            terrain_geometry_generator::create_tangent_space(container, filename);
        }
    }
    
    void terrain_geometry_generator::create_tangent_space(const std::shared_ptr<terrain_container>& container, const std::string& filename)
    {
        for(ui32 i = 0; i < container->get_chunks_num().x; ++i)
        {
            for(ui32 j = 0; j < container->get_chunks_num().y; ++j)
            {
                ...
                terrain_geometry_generator::generate_tangent_space(container, index);
            }
        }
        
        std::ofstream stream;
        stream.open(terrain_loader::get_tangent_space_mmap_filename(filename), std::ios::binary | std::ios::out | std::ios::trunc);
        if(!stream.is_open())
        {
            assert(false);
        }
    ...
    void terrain_geometry_generator::generate_tangent_space(const std::shared_ptr<terrain_container>& container, ui32 index)
    {
        ...
        ui16* indices = container->get_ibo_mmap(index, terrain_container::e_heigtmap_chunk_lod_01)->get_source_pointer();
        ui32 num_indices = container->get_ibo_mmap(index, terrain_container::e_heigtmap_chunk_lod_01)->get_size();
        ...
        for(i32 i = minmax.first; i <= minmax.second; i++)
        {
            ...
            tangent = terrain_geometry_generator::ortogonalize(glm::vec3(normal.x, normal.y, normal.z), tangent);
    ...
    glm::vec3 terrain_geometry_generator::get_closest_point_on_line(const glm::vec3& a, const glm::vec3& b, const glm::vec3& p)
    {
        glm::vec3 c = p - a;
        glm::vec3 v = b - a;
        f32 d = v.length();
    ...
    glm::vec3 terrain_geometry_generator::ortogonalize(const glm::vec3& v1, const glm::vec3& v2)
    {
        glm::vec3 v2ProjV1 = terrain_geometry_generator::get_closest_point_on_line( v1, -v1, v2 );
        glm::vec3 res = v2 - v2ProjV1;
        res = glm::normalize(res);
        return res;
    }
    
    void terrain_geometry_generator::generate_smooth_texcoord(const std::shared_ptr<terrain_container> &container, const std::string &filename)
    {
        terrain_geometry_generator::create_smooth_texcoord(container, filename);
    }
    
    void terrain_geometry_generator::create_smooth_texcoord(const std::shared_ptr<terrain_container> &container, const std::string &filename)
    {
        for(ui32 i = 0; i < container->get_chunks_num().x; ++i)
        {
            for(ui32 j = 0; j < container->get_chunks_num().y; ++j)
            {
                ui32 index = i + j * container->get_chunks_num().x;
                terrain_geometry_generator::generate_smooth_texcoord(container, index);
            }
        }
    }
    
    void terrain_geometry_generator::generate_smooth_texcoord(const std::shared_ptr<terrain_container>& container, ui32 index)
    {
        vbo::vertex_attribute* vertices = container->get_vbo_mmap(index)->get_pointer();
        ui32 num_vertices = container->get_vbo_mmap(index)->get_size();
    ...
    void terrain_geometry_generator::generate_attaches_to_vbo(const std::shared_ptr<terrain_container> &container, const std::string &filename)
    {
        if(!terrain_loader::is_attaches_to_vbo_mmap_exist(filename))
        {
            terrain_geometry_generator::create_attaches_to_vbo(container, filename);
        }
    }
    
    void terrain_geometry_generator::create_attaches_to_vbo(const std::shared_ptr<terrain_container> &container, const std::string &filename)
    {
        glm::ivec2 vertices_offset(0);
        vbo::vertex_attribute vertex;
        for(ui32 i = 0; i < container->get_chunks_num().x; ++i)
        ...
        std::ofstream stream;
        stream.open(terrain_loader::get_attaches_to_vbo_mmap_filename(filename), std::ios::binary | std::ios::out | std::ios::trunc);
        if(!stream.is_open())
        {
            assert(false);
        }
    ...
    void terrain_geometry_generator::update_vertices(const std::shared_ptr<terrain_container>& container, const std::vector<glm::vec3>& vertices)
    {
