...
namespace gb
{
    ...
    glm::mat4* animation_mixer::get_transformations() const
    {
        for(ui32 i = 0; i < m_skeleton->get_num_bones(); ++i)
        {
            bone_shared_ptr bone = m_skeleton->get_bone(i);
            if(bone->get_transformation())
            {
                m_bones_transformations[i] = (*bone->get_transformation());
            }
            else
            {
                m_bones_transformations[i] = glm::mat4(1.f);
            }
        }
    ...
    bool animation_mixer::is_animated(void) const
    {
